...
def create_UNet(data_lmdb, label_lmdb, batch_size, mean_file, phase):
    ...
    n.data = L.Data(source=data_lmdb, backend=P.Data.LMDB, batch_size=batch_size, ntop=1,
                    transform_param=dict(mean_file=mean_file), include=dict(phase=phase))
    n.label = L.Data(source=label_lmdb, backend=P.Data.LMDB, batch_size=batch_size, ntop=1,
                     transform_param=dict(mean_value=0), include=dict(phase=phase))
    ...
    n.acc, accuracy_by_class = L.Accuracy(n.score, n.labelcrop, accuracy_param={'axis': 1},
...
def write_net():
    if os.path.exists(TRAIN_PROTOTXT):
        os.remove(TRAIN_PROTOTXT)

    if os.path.exists(TEST_PROTOTXT):
        os.remove(TEST_PROTOTXT)

    with open(TRAIN_PROTOTXT, 'w') as f:
        # (data_lmdb, label_lmdb, batch_size, mean_file, phase)
        f.write(str(create_UNet(TRAIN_DATA_LMDB, TRAIN_LABEL_LMDB, BATCHSIZE, TRAIN_MEAN_FILE, caffe.TRAIN)))

    with open(TEST_PROTOTXT, 'w') as f:
        f.write(str(create_UNet(TEST_DATA_LMDB, TEST_LABEL_LMDB, BATCHSIZE, TRAIN_MEAN_FILE, caffe.TEST)))

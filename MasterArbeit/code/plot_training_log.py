...
def create_field_index():
    train_key = 'Train'
    test_key = 'Test'
    field_index = {train_key: {'Iters': 0, 'Seconds': 1, train_key + ' learning rate': 2, train_key + ' loss': 3},
                   test_key: {'Iters': 0, 'Seconds': 1, test_key + ' learning rate': 2, test_key+' acc_by_class': [3,4,5], test_key + ' acc': 6, test_key + ' loss': 8, test_key + ' acc_loss': [6,8]}}
    fields = set()
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if line[0] != '#':
                ...
                if type(field_idx1)!= int:
                    for l in field_idx1:
                        data[1].append(float(fields[l].strip()))
                else:
...
def get_solver_info(path_to_solver):
    regex_baselr = re.compile('base_lr')
    regex_weight_decay = re.compile('weight_decay')
    with open(path_to_solver) as f:
        for line in f:
            baselr_match = regex_baselr.search(line)
            weight_decay__match = regex_weight_decay.search(line)
            if baselr_match:
                base_lr = line.strip().split(':')[1]
            if weight_decay__match:
                weight_decay = line.strip().split(':')[1]
        return base_lr, weight_decay
        
def plot_chart(chart_type, path_to_png, path_to_solver, path_to_log_list):
    for path_to_log in path_to_log_list:
        ...
        use_marker = False
        # use_marker = True
        base_lr, weight_decay = get_solver_info(path_to_solver)
        legendtitle = r'$\eta$ :'+str(base_lr)+r', $\lambda$ :'+ str(weight_decay)
        data_x = data[0][0:len(data[1]):1]
        if y_axis_field == 'Test acc_by_class':
            data_acc_0 = data[1][0:len(data[1]):3]
            data_acc_1 = data[1][1:len(data[1]):3]
            data_acc_2 = data[1][2:len(data[1]):3]
            if not use_marker:                
                plt.plot(data_x, data_acc_0, label='Background', color=[1,0,0],
                         linewidth=linewidth)
                plt.plot(data_x, data_acc_1, label='MSCs', color=[0,1,0],
                         linewidth=linewidth)
                plt.plot(data_x, data_acc_2, label='Dead Cells', color=[0,0,1],
                         linewidth=linewidth)
            else:
        ...
        elif y_axis_field == 'Test acc_loss':
            data_acc_0 = data[1][0:len(data[1]):2]
            data_acc_1 = data[1][1:len(data[1]):2]
            if not use_marker:                
                plt.plot(data_x, data_acc_0, label='test accuracy', color=[1,0,0],
                         linewidth=linewidth)
                plt.plot(data_x, data_acc_1, label='test loss', color=[0,1,0],
                         linewidth=linewidth)
            else:
                marker = random_marker()
                plt.plot(data_x, data_acc_0, label='test accuracy', color=[1,0,0],
            ...
            data_y = data[1][0:len(data[1]):1]
            if not use_marker:
                plt.plot(data_x, data_y, label=y_axis_field, color=color,
                         linewidth=linewidth)
            else:
                marker = random_marker()
                plt.plot(data_x, data_y, label=y_axis_field, color=color,
    ...
    max_y = max(data[1]) + 1 if max(data[1]) >1 else 1
    
    plt.legend(title= legendtitle,loc=legend_loc, ncol=1)  # ajust ncol to fit the space
...
if __name__ == '__main__':
    if len(sys.argv) < 5:
        ...
        plot_chart(chart_type, path_to_png, path_to_solver, path_to_logs)
